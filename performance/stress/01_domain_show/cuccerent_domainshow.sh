...
EOF
  ...
  rhc app create app python-2.6 -l user$i -predhat --no-git
  rhc app stop app -l user$i -predhat
  rhc app create app$i jbosseap-6 -l user$i -predhat --no-git
...
do
  ...
  for i in `seq 1 $j`; do (time -p rhc domain show -l user$i -predhat) 2>>domaintest/domain_show$j  & done
  sleep 60
done
...
do
  ...
  cat domaintest/domain_show$f | grep real | awk '{sum+=$2}END{print sum/NR}'  >> test_results
  echo "" >> test_results
  sleep 1
done
